...
class BookDetails(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if user.is_authenticated:
            canReview = BorrowedBook.objects.filter(user=user, book=book).exists()
        context["related_books"] = Book.objects.all()[0:5]
        context['form'] = ReviewForm
        context['canReview'] = canReview
        context['reviews'] = Reviews.objects.filter(book=book).order_by('-rating')
        return context
    ...
    def post(self, request, slug):
        ...
        if form.is_valid():
            ...
            Reviews.objects.create(user=user, book=book,rating=rating, review=review)
            messages.success(self.request, f"Review posted !!")
...
class BorrowBook(LoginRequiredMixin, View):
    def post(self, request, slug):
        ...
        if isAvailable:
            messages.success(self.request, f"Already borrowed this book!!")
        elif user_account.balance < book_price:
            messages.success(self.request, f"not enough balance to borrow this book")
        else:
            ...
            messages.success(self.request, f"Successfully borrowed this book")
